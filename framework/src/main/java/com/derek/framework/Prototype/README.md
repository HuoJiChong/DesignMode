...
# 小结
原型模式本质上就是对象拷贝，与C++中的拷贝够着函数有些类似，他们之间容易出现的问题也就是深拷贝，浅拷贝。
使用原型模型可以解决构造复杂对象的资源消耗问题，能够在某些场景下提升构造对象的效率。还有一个重要的用途就是保护性拷贝，也就是某个对象对外可能是只读的，
为了防止外部对这个只读对象修改，通常可以返回一个对象拷贝的形式实现只读的限制。

原型模式是在内存中二进制流的拷贝，要比直接new一个对象性能好很多，特别是要在一个循环体内产生大量的对象是，原型模式可以很好的体现其有点。
